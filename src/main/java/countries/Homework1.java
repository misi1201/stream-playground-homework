...
public class Homework1 {
    ...
    public double streamPipeline12() {
        return countries.stream().mapToDouble(country -> country.getName().length()).average().getAsDouble();
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().doubleValue() < 1).map(Country::getName).forEach(System.out::println);
    }
